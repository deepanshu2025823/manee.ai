        ...
        <div className="flex-1 overflow-y-auto px-5 py-2 space-y-4 bg-white scrollbar-thin scrollbar-thumb-gray-200">
          {messages.map((msg, index) => (
            <div key={index} className={`flex w-full ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
              {msg.role === "bot" && (
                 <div className="w-7 h-7 rounded-full bg-gray-100 border border-gray-200 flex items-center justify-center mr-2 shrink-0 mt-1"><span className="text-[#EB2328] font-bold text-[10px]">M</span></div>
              )}
              <div className={`max-w-[85%] p-3 text-[14px] leading-snug shadow-sm relative ${msg.role === "user" ? "bg-[#EB2328] text-white rounded-2xl rounded-tr-sm" : "bg-[#F7F7F7] text-gray-800 rounded-2xl rounded-tl-sm border border-gray-100"}`}>
                <ReactMarkdown remarkPlugins={[remarkGfm]} components={{
                    ...
                    a: ({node, ...props}) => <a className="underline text-blue-600" target="_blank" {...props} />,
          ...
          {loading && (
            <div className="flex justify-start w-full">
               ...
               <div className="bg-[#F7F7F7] px-4 py-3 rounded-2xl rounded-tl-sm border border-gray-100 flex gap-1 items-center"><div className="w-1.5 h-1.5 bg-gray-400 rounded-full animate-bounce"></div><div className="w-1.5 h-1.5 bg-gray-400 rounded-full animate-bounce delay-100"></div><div className="w-1.5 h-1.5 bg-gray-400 rounded-full animate-bounce delay-200"></div></div>
            </div>
          )}
        ...
        <div className="p-4 bg-white shrink-0">
          <div className="flex flex-col items-end gap-2 mb-3">
             {quickReplies.map((reply, i) => (
               <button key={i} onClick={() => sendMessage(reply)} className="px-3 py-1.5 bg-white border border-gray-300 text-gray-700 text-xs font-medium rounded-full hover:border-[#EB2328] hover:text-[#EB2328] transition shadow-sm w-fit">{reply}</button>
             ))}
          </div>
